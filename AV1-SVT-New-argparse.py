...
#================================================
# CPU Core Pinning
# Make ffmpeg only use some of the core's of your CPU leaving the rest for other tasks. No effect on file sizes just CPU %. It will however increase encode time.
# True  for slower dont turn my CPU into an infero while encoding
# False for full throttle torch my CPU encoding. :-)
PIN_CORES = True
CPU_CORE_PINNING = "0-7" # define the core's you want used as in 0-7 <--- (1st 8 cores)
...
#================================================
# Log path constants
LOG_DIR = os.path.expanduser('~/AV1-SVT_log')
LOG_FILE_NAME = 'AV1-SVT.log'
#================================================
...
"""
    ...
    parser.add_argument('--pin-cores', dest='pin_cores', action='store_true', default=PIN_CORES,
                        help='Enable CPU core pinning (default: %(default)s).')
    parser.add_argument('--no-pin-cores', dest='pin_cores', action='store_false', default=argparse.SUPPRESS, help='Disable CPU core pinning.')
    parser.add_argument('--cpu-cores', default=CPU_CORE_PINNING,
                        help='CPU core range to pin ffmpeg to (e.g. 0-7). Only takes effect when --pin-cores is active.')
    ...
    parser.add_argument('--crf', type=ranged_int(1, 70), default=CRF,
                        help='CRF value for SVT-AV1 (1-70).')
    parser.add_argument('--preset-svt', type=ranged_int(0, 13), default=PRESET_SVT,
                        help='SVT-AV1 preset (0-13). Lower = better quality, slower encode.')
    parser.add_argument('--film-grain', type=ranged_int(0, 50), default=FILM_GRAIN,
...
def run_ffmpeg_with_size_estimation(cmd, original_duration, original_size, output_file, file_num=1, total_files=1):
    ...
    if PIN_CORES:
        print(f"{colors.orange}Processing with CPU Pinning on Cores {CPU_CORE_PINNING}{colors.reset}", flush=True)
    else:
        print(f"{colors.orange}Processing with NO CPU Core Pinning{colors.reset}", flush=True)
...
def encode_video(source, dest, filename, file_num=1, total_files=1):
    ...
    try:
        ...
        if SCALE == 0:
            output_file = os.path.join(dest, f"{os.path.splitext(filename)[0]}-AV1-No-Scaling.mkv")
        else:
            output_file = os.path.join(dest, f"{os.path.splitext(filename)[0]}-AV1-{SCALE}p.mkv")
        ...
        print(f"{colors.pink}Preset Type............:\t{PRESET_SVT}{colors.reset}")
        ...
        cmd = [
            ...
            '-c:v', 'libsvtav1',
            '-pix_fmt', 'yuv420p10le',
            '-preset', str(PRESET_SVT),
            '-svtav1-params', f'tile-columns=0:tile-rows=0:scd=1:tune=2:enable-qm=1:qm-min=0:qm-max=15:film-grain={FILM_GRAIN}',
            '-crf', str(CRF),
            '-g', '120',
        ]
        ...
        prefix = []
        if PIN_CORES:
            prefix = ['taskset', '-c', str(CPU_CORE_PINNING)]
        prefix += ['nice', '-n', '19', 'ionice', '-c3']
        cmd = prefix + cmd
        ...
        if encoding_started and 'output_file' in locals():
                print(f"{colors.orange}*** Removing Unfinished AV1 Video ***{colors.reset}")
...
def reduce_file_size(input_file, output_file, video_duration, start_time, elapsed_time, input_res, scale, video_codec='Unknown', audio_codec='Unknown', preserve_subtitles=False, file_has_chapters=False):
    ...
    file_logger.info(f"Preset Value...........:\t{PRESET_SVT}")
...
def main():
    global file_logger, PIN_CORES, CPU_CORE_PINNING, SCALE, CRF, PRESET_SVT, FILM_GRAIN

    args = parse_arguments()
    PIN_CORES = args.pin_cores
    CPU_CORE_PINNING = args.cpu_cores
    SCALE = args.scale
    CRF = args.crf
    PRESET_SVT = args.preset_svt
    ...
    if os.path.isdir(source) and os.path.isdir(dest):
        # Batch mode for directory encoding (AV1-SVT)
        if PIN_CORES:
        ...
        # Single file mode encoding (AV1-SVT)
        if PIN_CORES:
            print(f"{colors.red}Single File mode encoding (AV1-SVT){colors.yellow} CPU Core Pinning is Active, Cores {CPU_CORE_PINNING}{colors.reset}")
        else:
            print(f"{colors.red}Single File mode encoding (AV1-SVT){colors.yellow} CPU Core Pinning is NOT Active{colors.reset}")
